...
const TARGET_NETWORK_ERROR_MESSAGE = (
  <>
    <p className="font-bold mt-0 mb-1">
      <code className="italic bg-base-300 text-base font-bold"> targeNetwork </code> is not localhost
    </p>
    <p className="m-0">
      - You are on <code className="italic bg-base-300 text-base font-bold">{getTargetNetwork().name}</code> .This block
...
const Blockexplorer: NextPage = () => {
  ...
  useEffect(() => {
    if (getTargetNetwork().id === hardhat.id && error) {
      notification.error(LOCALHOST_ERROR_MESSAGE);
    }

    if (getTargetNetwork().id !== hardhat.id) {
      notification.error(TARGET_NETWORK_ERROR_MESSAGE);
    }
  }, [error]);
